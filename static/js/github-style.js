window.addEventListener('load', _ => {
  ...
  function yearList() {
    ...
    for (let i = 0; i < years.length; i++) {
      ...
      node.innerHTML = `<a class="js-year-link js-onclick-switch-year filter-item px-3 mb-2 py-2" postyear="${year}">${year}</a>`;
      document.querySelector('#year-list').appendChild(node);
    }
  
    const switch_year_buttons = document.querySelectorAll('.js-onclick-switch-year');
    for (let i = 0; i < switch_year_buttons.length; i++) {
      const element = switch_year_buttons[i];
      element.addEventListener('click', e => {
        const year = e.target.getAttribute('postyear');
        switchYear(year);
      });
    }
  }
  ...
  function graph(year, posts, startDate, endDate) {
    ...
    for (let i = 0; i < 53; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        fill="${color}" postcount="${c}" postdate="${dataDate}"></rect>`;
      }
      html += '</g>';
    }
    ...
    let days = document.querySelectorAll('.day');
      for (let i = 0; i < days.length; i++) {
        ...
        element.addEventListener('mouseover', (e) => {
          const day = e.target;
          let count = day.getAttribute('postcount');
          let date = day.getAttribute('postdate');
          svgTip(day, count, date);
        });
